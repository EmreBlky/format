...
<?php
							...
							if($searchtype!="imei"){
									if($username!=""){
										$userData=select_query_live("select id,phone_number from matrix.users where sys_username='".$username."'");
...
//									$time=$time+19800;
	...
	$fp=fopen($path,'r');
	 ...
	 if(isset($_POST['submit']))
		...
		$data=select_query_live("select id,sys_simcard from matrix.devices where imei='".$_POST['TxtImei']."'");
            $service_data=select_query_live('select id,log,veh_reg from matrix.services where sys_device_id =('.$data[0]['id'].')');
