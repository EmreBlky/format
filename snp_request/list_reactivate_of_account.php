...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=reactiavteReasonComment",
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=reactivateaccountbackComment",
...
<div class="top-bar">
  ...
  <!--<a href="reactivation_of_account.php" class="button">ADD NEW </a>-->
...
if($_POST["Showrequest"]==1)
 ...
 { 
   $WhereQuery=" where (approve_status=0 or (approve_status=1 and support_comment!='' and final_status!=1)) and (sales_comment is null or reactivation_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
 }
...
?>
	...
	$rowuser=select_query($sql);
        ...
        <td><?php echo $query[$i]["deact_reason"];?></td>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'reactivation_of_account','popup1'); " class="topopup">View Detail</a> 
          <!--|<a href="#" onclick="return reasonComment(<?php echo $query[$i]["id"];?>);"  >Reason</a>-->
	...
	if(( $query[$i]["support_comment"]!="" or $query[$i]["admin_comment"]!="" or $query[$i]["pay_pending"]!="") && $query[$i]["final_status"]!=1) {?>
          <a href="reactivation_of_account.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
