...
class master
{
		...
		function getVimalDetails($UserId)
		{
			...
			$this->data = select_query_live_con("select sys_username from matrix.users where id=(select sys_user_id from matrix.group_services join matrix.group_users on group_services.sys_group_id=group_users.sys_group_id where group_services.sys_service_id='".$UserId."' and sys_user_id not in (2143,3052,3053,3068,3070,3073,3081) limit 1)");
        ...
        function getdebug_data($userId,$selecttype)
        {
				...
				$this->data = select_query_live_con("select latest_telemetry.gps_fix,latest_telemetry.tel_voltage,case when tel_poweralert=true then true else false end as poweronoff,services.id,sys_created,veh_reg,adddate(latest_telemetry.gps_time,INTERVAL 19800 second) as lastcontact,round(gps_speed*1.609,0) as speed,  case when tel_ignition=true then true else false end as aconoff , geo_street as street, latest_telemetry.gps_latitude as lat,latest_telemetry.gps_longitude as lng ,devices.imei,services.device_removed_service from matrix.services    join matrix.latest_telemetry on latest_telemetry.sys_service_id=services.id join matrix.devices on devices.id=services.sys_device_id  where services.id in   (select distinct sys_service_id from matrix.group_services where active=true and sys_group_id in ( select sys_group_id from matrix.group_users where sys_user_id=(".$userId."))) and adddate(latest_telemetry.gps_time,INTERVAL 19800 second)< adddate('".date('Y-m-d')." 09:00:00',interval -2 hour)");
		...
		function get_Snp_debug_data($userId,$selecttype)
        {
