...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where user_id IN($user) and billing='Yes'";
 }
 else
 { 
	  
	 $WhereQuery=" where user_id IN($user) and (approve_status=0 and final_status!=1) and (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
?>
          ...
          <td><?php if( $_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2 ){?>
            <a href="#" onclick="return backComment(<?php echo $query[$i]["id"];?>);"  >Add Details</a>
            <?php }?></td>
        </tr>
        <?php }?>
    </table>
