...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=StartUserGps",
 ...
 <form name="myformlisting" method="post" action="">
  <select name="Showrequest" id="Showrequest" onchange="form.submit();" >
            ...
            <option value="2" <?if($_POST['Showrequest']==2){ echo "Selected" ;}?>>All</option>
                    ...
                    <a href="stop_gps.php" class="button">ADD NEW </a>
                    <h1>Stop GPS</h1>
...
if($_POST["Showrequest"]==1)
 ...
 { 
   $WhereQuery=" where (approve_status=0 or (approve_status=1 and support_comment!='' and final_status!=1)) and (sales_comment is null or stop_gps_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
 }
...
?>
 ...
 <? $sql="SELECT Userid AS id,UserName AS sys_username FROM addclient  WHERE Userid=".$query[$i]["client"];
  ...
  <td><?php echo $query[$i]["reason"];?></td>
   <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'stop_gps','popup1'); " class="topopup">View Detail</a>
   <?php //if($_POST["Showrequest"]==1 && $query[$i]["start_gps"]=='N'){?>
   <!--| <a href="#" onclick="return ConfirmReactivate(<?php echo $query[$i]["id"];?>);"  >StartGPS</a>-->
   <? //}
...
<? }?>
  ...
  <td><?php 
if(( $query[$i]["support_comment"]!="" or $query[$i]["admin_comment"]!="" or $query[$i]["total_pending"]!="") && $query[$i]["final_status"]!=1 ) {?>   
	<a href="stop_gps.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a>
