...
<div class="top-bar">
  ...
  <a href="discounting.php" class="button">ADD NEW </a>
...
if($_POST["Showrequest"]==1)
 ...
 { 
    $WhereQuery="where (((approve_status=0) or (approve_status=1 and support_comment!='')) and final_status!=1) and (sales_comment is null or discount_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
 }
...
if( $query[$i]["forward_comment"]!="" && $query[$i]["forward_req_user"]==$_SESSION["user_name"]){ ?>
        ...
        <td><?php if($query[$i]["approve_status"]!=1) {?>
          <a href="discounting.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a> 
