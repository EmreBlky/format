...
<div class="top-bar">
  ...
  <a href="no_bill.php" class="button">ADD NEW </a>
  <h1>No Bill</h1>
</div>
<div class="top-bar">
...
if($_POST["Showrequest"]==1)
 ...
 {
   $WhereQuery=" where (approve_status=0 or (approve_status=1 and support_comment!='' and final_status!=1)) and (sales_comment is null or no_bill_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
 }
...
?>
    ...
    $rowuser=select_query($sql);
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'no_bills','popup1'); " class="topopup">View Detail</a>
          ...
          <? }?></td>
        <td><?php if(( $query[$i]["support_comment"]!="" or $query[$i]["admin_comment"]!="" or $query[$i]["total_pending"]!="") && $query[$i]["final_status"]!=1 ) {?>
          <a href="no_bill.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a> 
