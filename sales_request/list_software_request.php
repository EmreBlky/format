...
<div class="top-bar">
  ...
  <a href="software_request.php" class="button">ADD NEW </a>
...
if($_POST["Showrequest"]==1)
 ...
 { 
   $WhereQuery=" where (approve_status=0 or (approve_status=1 and support_comment!='' and final_status!=1)) and (sales_comment is null or software_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
 }
...
if(( $query[$i]["support_comment"]!="" or $query[$i]["admin_comment"]!="") && $query[$i]["final_status"]!=1 ) {?>
          <a href="software_request.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a> 
