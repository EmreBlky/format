...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=ReactivationPaymentClear",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=ReactivationPaymentPending",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where  acc_manager in ('khetraj','jaipursales') ";
 }
 ...
 { 
	...
	/* $WhereQuery=" where approve_status=0 and final_status!=1 and acc_manager in ('khetraj','jaipursales')";*/
	   ...
	   $WhereQuery=" where (approve_status=0 or (approve_status=1 and support_comment!='' and final_status!=1)) and (sales_comment is null or reactivation_status=2) and (acc_manager in ('khetraj','jaipursales') or (forward_req_user in ('khetraj','jaipursales') and (forward_back_comment is null or forward_back_comment='')))";
...
$query = mysql_query("SELECT * FROM reactivation_of_account ". $WhereQuery."  order by id desc ");   
...
?>
 ...
 <? $sql="SELECT Userid AS id,UserName AS sys_username FROM addclient  WHERE Userid=".$row["user_id"];
  ...
  <td><?php echo $row["reason"];?></td>
   ...
   <td><a href="#" onclick="Show_record(<?php echo $row["id"];?>,'reactivation_of_account','popup1'); " class="topopup">View Detail</a></td>
   <td><?php 
if($row["approve_status"]==0) {?>  <a href="reactivation_of_account.php?id=<?=$row['id'];?>&action=edit<? echo $pg;?>">Add Details</a>
