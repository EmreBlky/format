...
if($_POST["Showrequest"]==1)
 ...
 { 
   ...
   /*$WhereQuery=" where approve_status=0 and discount_issue='Service Issue' and acc_manager in ('khetraj','jaipursales')";*/
...
$query = mysql_query("SELECT * FROM discount_details  ". $WhereQuery."  order by id desc ");   
...
?>
 ...
 <? $sql="SELECT Userid AS id,UserName AS sys_username FROM addclient  WHERE Userid=".$row["user"];
  ...
  <? 
  if($row["rent_device"]=='Rent') {
  $Month_device=$row["mon_of_dis_in_case_of_rent"];
  }
  elseif($row["rent_device"]=='Device') {
  $Month_device='Device';
  }
  else {
  ...
  <td bgcolor="#FF0000"><?php echo $row["dis_amt"];?></td>  
  <td><?php echo $row["reason"];?></td>
   <td><a href="#" onclick="Show_record(<?php echo $row["id"];?>,'discount_details','popup1'); " class="topopup">View Detail</a>  
  </td><td><?php 
if( $row["discount_issue"]=="Service Issue" ) {?>   <a href="discounting.php?id=<?=$row['id'];?>&action=edit<? echo $pg;?>">Add Comments</a>
