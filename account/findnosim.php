...
$query="SELECT * FROM no_vehicle_sim";
 ...
 if($no_sim=='highbill_sim')
	 {   
	$query="SELECT * FROM sim_no WHERE NOT EXISTS (SELECT * FROM voice_sim WHERE voice_sim.sim_no = sim_no.sim_no) and amt>56";
...
$query="SELECT sim_no,COUNT(sim_no) AS tagsim_no FROM no_vehicle_sim GROUP BY amt=0";

$result1=mysql_query($query);
}

  if($no_sim=='highbill_sim') {
$query="SELECT sim_no,COUNT(sim_no) AS tagsim_no FROM sim_no WHERE NOT EXISTS (SELECT * FROM voice_sim WHERE voice_sim.sim_no = sim_no.sim_no) and amt>56 GROUP BY amt>56";
$result1=mysql_query($query);
}
