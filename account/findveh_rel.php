...
$query="SELECT sim_no,veh_no FROM reliance_client WHERE client='$veh'";
...
$query="SELECT veh_no,COUNT(DISTINCT veh_no) AS tagveh_no FROM reliance_client WHERE client='$veh' GROUP BY client";
...
$query="SELECT sum(amt) as total FROM reliance_sim WHERE sim_no in (".substr($simlist,0,-1).")";
