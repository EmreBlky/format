...
if(isset($_POST["submit"]))
{
    ...
    if($TxtReason=="Client Request")
    {
   		$Client_request = $_POST["TxtReason_comment"]; 
    }
	
	$new_veh_len = strlen($_POST["New_Veh_num"]);
  ...
  if($action=='edit')
    {
		...
		if($new_veh_len > 2)
		{
			...
			$query="update vehicle_no_change set sales_manager='".$sales_manager."',new_reg_no='".$New_Veh_num."',billing='".$billing."',billing_reason='".$reason."',numberchange_date='".$rdd_date."',reason='".$TxtReason."',vehicle_reason='".$Client_request."' where id=$id";
		...
		{
			echo "<p><h3 style='color: rgb(242, 11, 11); text-align: center;'>Kindly check New vehicle no.</h3></p>";
		}
}
 ...
 <form name="myForm" action="" onsubmit="return validateForm()" method="post">
   ...
   <table style=" padding-left: 100px;width: 500px;" cellspacing="5" cellpadding="5">
       ...
       <!-- <tr>
            ...
            <?php
           /* $main_user_id=mysql_query("SELECT Userid AS user_id,UserName AS `name` FROM addclient WHERE sys_active=1 ORDER BY `name` ASC",$dblink2);
            while ($data=mysql_fetch_assoc($main_user_id))
            {*/
            ?>
           
             <option name="main_user_id" value="<?=$data['user_id']?>" <? if($result[0]['user_id']==$data['user_id']) {?> selected="selected" <? } ?> >
            <?php //echo $data['name']; ?>
            </option>
            <?php
            //}
        ...
        <tr>
           ...
           Client User Name</td>
            ...
            //$data = mysql_fetch_assoc($main_user_iddata);?>
            	<?php echo $main_user_iddata[0]['name'];?>
            </td>
        </tr>
        ...
        <tr>
           ...
           <td>
                ...
                <Input type = 'radio' Name ='billing' id="billing" value= 'Yes' <?php if($result[0]['billing']=="Yes"){echo "checked=\"checked\""; }?>
                onchange="Status(this.value)" >Yes
                
                <Input type = 'radio' Name ='billing' id="billing" value= 'No' <?php if($result[0]['billing']=="No"){echo "checked=\"checked\""; }?>
                onchange="Status(this.value)" >No
        </td>
    </tr>
    <tr>
    	<td colspan="2">
   ...
   <!--</table>
    ...
    <table style=" padding-left: 100px;width: 500px;" cellspacing="5" cellpadding="5">-->
              ...
              <tr>
                  ...
                  <td>
                    <select name="vehicle_change_reason" id="vehicle_change_reason" onchange="RequestComment(this.value)">
              ...
              <tr>
                <td colspan="2">
                    <table id="ClientRequest"  style="width: 300px;display:none;border:1" cellspacing="5" cellpadding="0">
                        <tr><td> <label  id="lblReason">Clent Request Comment</label></td>
                         	<td> <textarea rows="5" cols="25"  type="text" name="TxtReason_comment" id="TxtReason_comment" ><?=$result[0]['vehicle_reason']?></textarea></td>
                        </tr>
                    </table>
             </td>
            </tr>
