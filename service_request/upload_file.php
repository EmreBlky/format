...
if((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
  ...
  if (($ext == "jpg") && ($_FILES["uploaded_file"]["type"] == "image/jpeg") && 
    ...
    //Determine the path to which we want to save this file
      $newname = __DOCUMENT_ROOT.'/Pancard/'.$filename;
...
if((!empty($_FILES["uploaded_file_add"])) && ($_FILES['uploaded_file']['error'] == 0)) {
  ...
  $ext = substr($filename, strrpos($filename, '.') + 1);
  if (($ext == "jpg") && ($_FILES["uploaded_file_add"]["type"] == "image/jpeg") && 
    ($_FILES["uploaded_file_add"]["size"] < 3500000)) {
    //Determine the path to which we want to save this file
      $newname = __DOCUMENT_ROOT.'/Addressproof/'.$filename;
      //Check if the file with the same name is already exists on the server
      if (!file_exists($newname)) {
        //Attempt to move the uploaded file to it's new place
