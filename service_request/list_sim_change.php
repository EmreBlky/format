...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=simchangeserviceComment",
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=SimcloseComment",
...
if($_POST["Showrequest"]==1)
 ...
 { 
	  //$WhereQuery=" where ((approve_status=0 or approve_status=2) or (approve_status=1 and final_status!=1 and support_comment!='')) and (service_comment is null and forward_back_comment is null) and (acc_manager='".$_SESSION['user_name']."' or forward_req_user='".$_SESSION["user_name"]."')";
	  
	  $WhereQuery=" where ((approve_status=0 and final_status!=1) or (approve_status=1 and final_status!=1 and support_comment!='')) and (service_comment is null or sim_change_status=2) and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
...
<a href="sim_change.php?id=<?=$query[$i]['id'];?>&action=edit<? echo $pg;?>">Edit</a>
