...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Renewdimts_imeiclose",
 		data:"row_id="+row_id,
...
function PortChange(row_id)
{
  var Y = confirm("Are you sure you want to Change Port?");
  if (Y)
  {
  Change(row_id);
      return ture;
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Port_change",
 		data:"row_id="+row_id,
		success:function(msg){
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=renew_serviceComment",
...
<div class="top-bar">
  ...
  <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    <tr align="right">
      <td> User Name</td>
      <td><select name="main_user_id" id="TxtMainUserId">
          ...
          <?php
			...
			for($u=0;$u<count($main_user_iddata);$u++)
					{
			?>
          <option name="main_user_id" value="<?=$main_user_iddata[$u]['name']?>" > <?php echo $main_user_iddata[$u]['name']; ?> </option>
          <?php 
								} 
...
else if($_POST["Showrequest"]==4)
{
 ...
 { 
	 ...
	 $WhereQuery=" where (((approve_status=0 and (service_comment is null or renew_dimts_status=2)) or approve_status=1) and final_status!=1)  and (acc_manager='".$_SESSION['user_name']."' or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')))";
...
?>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'renew_dimts_imei','popup1'); " class="topopup">View Detail</a>
	...
	if( $query[$i]["forward_comment"]!="" && $query[$i]["forward_req_user"]!="" ){ ?>
          ...
          <?php }
	if( $query[$i]["port_change_status"]=="No"){
	?>
          | <a href="#" onclick="return PortChange(<?php echo $query[$i]["id"];?>);">Port Change</a>
          <?php } ?>
