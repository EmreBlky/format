  ...
  /*$qrycsv="select services.id as id,services.id,sys_created,veh_reg,adddate(latest_telemetry.gps_time,INTERVAL 19800 second) as lastcontact,round(gps_speed*1.609,0) as speed,  case when tel_ignition=true then true else false end as aconoff , geo_street as street, latest_telemetry.gps_latitude as lat,latest_telemetry.gps_longitude as lng ,devices.imei,services.device_removed_service from services 	join latest_telemetry on latest_telemetry.sys_service_id=services.id join devices on devices.id=services.sys_device_id 	join mobile_simcards on mobile_simcards.id=devices.sys_simcard  where services.id in  	(select distinct sys_service_id from group_services where active=true and sys_group_id in ( select sys_group_id from group_users where sys_user_id=(".$userId."))) and adddate(latest_telemetry.gps_time,INTERVAL 19800 second)< adddate(now(),interval -2 hour) and adddate(latest_telemetry.gps_time,INTERVAL 19800 second)<'".date('Y-m-d')."' order by lastcontact DESC"; 
			...
			//while($row=mysql_fetch_array($query)) 
				{ 
				 ...
				 /*$qrycom="select comment,comment_by,date from `comment` where service_id=".$row['id']." order by date desc";				
				 $Commentquery = mysql_fetch_array(mysql_query($qrycom,$dblink));*/
...
$objWriter->save(__DOCUMENT_ROOT.'/service_request/'.$userId.'-'.date('Y-m-d').'.xls');

  

 header("location:".$userId."-".date('Y-m-d').".xls");
}
?>