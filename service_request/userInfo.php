...
{	
	...
	$deviceType=select_inventory_query($query);
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
          	...
          	$tools = explode('#',$row[0]['accessories_tollkit']);
          	$ac_tools = array();

          	for($i=0;$i<count($tools);$i++){

          		$acc_tools= select_query("select items FROM internalsoftware.toolkit_access where id='". $tools[$i]."'");

          		array_push($ac_tools,$acc_tools[0]['items']);

          		echo trim($ac_tools[$i]).",";
