...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=devicechangeclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=devicechangesupportComment",
...
<?php
 ...
 {
	 $WhereQuery=" where (user_id IN($user) and support_comment!='' and final_status=0 and (service_comment is null or device_change_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";
 } 
 else
 { 
	  
	 $WhereQuery=" where (user_id IN($user) and (approve_status=1 and final_status!=1 and device_change_status=1 and (support_comment is null or service_comment!=''))) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
<td style="width:200px"><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'device_change','popup1'); " class="topopup"> View Detail</a>
