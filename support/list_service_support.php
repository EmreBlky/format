...
if($_POST["Showrequest"]==1)
 {
	   $WhereQuery=" where fwd_tech_rm_id='".$_SESSION['userId']."' and (service_status='5' or service_status='6')";
 }
 ...
 {
	  $WhereQuery="where service_status='2' and fwd_tech_rm_id='".$_SESSION['userId']."' and fwd_repair_to_serv is NOT NULL ";
 }
 else if($_POST["Showrequest"]==4)
 {
	  $WhereQuery="where service_status='11' and fwd_tech_rm_id='".$_SESSION['userId']."' and (fwd_repair_id is NOT NULL and fwd_serv_to_repair is NOT NULL and fwd_repair_to_serv is NULL) ";
 }
 else
 { 
	 $WhereQuery=" where service_status='11' and fwd_tech_rm_id='".$_SESSION['userId']."' and fwd_reason is NOT NULL and fwd_repair_id is NULL and fwd_serv_to_repair is NULL and fwd_repair_to_serv is NULL ";
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
    ...
    ?>
      ...
      <tr align="Center"   <? if($query[$i]['service_status']==5 or $query[$i]['service_status']==6 )  {  ?> style="background:#CCCCCC;" <? }
        ...
        <td>&nbsp;<?php echo $query[$i]['veh_reg'];?></span></td>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'services','popup1'); " class="topopup">View Detail</a>
        ...
        <td>
        <?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2 && $_POST["Showrequest"]!=3 && $_POST["Showrequest"]!=4){?>
        <a href="forward_service_installation_iframe.php?id=<?=$query[$i]["id"];?>&req_id=1&height=220&width=480" class="thickbox">Forward</a>
         <?php } ?>
        </td>
      </tr>
      <?php  
