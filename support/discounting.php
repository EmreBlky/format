...
$Discount_Amount = $_POST["discount_repair_cost"]; 
 ...
 if($action=='edit')
	...
	$query="update discount_details set date='".$date."',acc_manager='".$account_manager."',sales_manager='".$sales_manager."',client='".$company."',user='".$main_user_id."',total_no_of_vehicles='".$tot_no_of_vehicles."',reason='".$TxtReason."',software_comment='".$software_comment."',discount_issue='".$discount_issue."' where id=$id";
...
function RentDeviceDiv(radioValue)
{
 ...
 <form name="myForm" action="" onSubmit="return validateForm()" method="post">
    ...
    <table style=" padding-left: 100px;width: 500px;" cellspacing="5" cellpadding="5">
        ...
        <option name="sales_manager" value="<?=$sales_manager[$s]['name']?>" <? if($result[0]['sales_person']==$sales_manager[$s]['name']) {?> selected="selected" <? } ?> >
...
<select name="main_user_id" id="TxtMainUserId"  onchange="showUser(this.value,'ajaxdata');gettotal_veh_byuser(this.value,'TxtTotalVehicle');getCompanyName(this.value,'TxtCompany');">
           ...
           <option name="main_user_id" value="<?=$main_user_id[$m]['user_id']?>" <? if($result[0]['user']==$main_user_id[$m]['user_id']) {?> selected="selected" <? } ?> >
  ...
  <option value="Client Side Issue" <? if($result[0]['discount_issue']=='Client Side Issue') {?> selected="selected" <? } ?> >Client Side Issue</option>
        ...
        <tr>
		 ...
		 <tr>
            <td class="style2">
                Software Comment</td>
            <td><textarea rows="5" cols="25"  type="text" name="software_comment" id="Txtsoftware_comment" ><?=$result[0]['software_comment']?></textarea>
                </td>
        </tr>
