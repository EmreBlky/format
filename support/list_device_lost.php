...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=device_lostclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=device_lostsupportComment",
...
<div class="table">
 ...
 {
	 $WhereQuery=" where (user_id IN($user) and support_comment!='' and final_status=0 and (service_comment is null or device_lost_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";

 } 
 else
 { 
	  
	 $WhereQuery=" where (user_id IN($user) and (approve_status=1 and final_status!=1 and device_lost_status=1 and (support_comment is null or service_comment!=''))) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
?>
        ...
        <td><?php echo $query[$i]["odd_imei"];?></td>
        <td><?php echo $query[$i]["ndd_imei"];?></td>
        <td><?php echo $query[$i]["reason"];?></td>
        <td  style="width:200px"><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'device_lost','popup1'); " class="topopup">View Detail</a>
