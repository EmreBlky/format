...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=discount_detailsclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=discount_detailssupportComment",
...
//$num=mysql_num_rows(mysql_query("SELECT * FROM new_account_creation order by id DESC"));
 ...
 {
	  $WhereQuery=" where user IN($user) and final_status=1 and discount_issue='Software Issue'";
 }
 else if($_POST["Showrequest"]==2)
 {
	 /*$WhereQuery=" where user IN($user) and (approve_status=1 or approve_status=0) and discount_issue='Software Issue' ";*/
	 $WhereQuery=" where user IN($user) and discount_issue='Software Issue'";
 }
 else if($_POST["Showrequest"]==4)
 {
	 $WhereQuery=" where (user IN($user) and (support_comment!='' or software_comment!='') and approve_status=0 and (sales_comment is null or discount_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";
 } 
 else if($_POST["Showrequest"]==6)
 {
	 $WhereQuery=" where (user IN($user) and forward_req_user!='' and forward_comment!='' and forward_by='".$_SESSION["user_name"]."')";
 }
 else
 { 
	  
	 $WhereQuery=" where (user IN($user) and (approve_status=0 and discount_issue='Software Issue' and discount_status=1 and (support_comment is null or sales_comment!='') and software_comment is null) and (forward_req_user is null or forward_back_comment!='')) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
?>
  ...
  ?>
        ...
        <td><?php echo $query[$i]["amt_rec_after_dis"];?></td>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'discount_details','popup1'); " class="topopup">View Detail</a>
          ...
          | <a href="#" onclick="return softwareComment(<?php echo $query[$i]["id"];?>);">Add Comment</a> | <a href="#" onclick="return BackComment(<?php echo $query[$i]["id"];?>);">Back Comment</a>
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=14&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
        
        <!-- <td>-->
        <?php //if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
        <?php //if( $query[$i]["discount_issue"]=="Software Issue" ) {?>
