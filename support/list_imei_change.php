...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=imei_changeclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=imei_changesupportComment",
...
$query = mysql_query("SELECT * FROM imei_change   ". $WhereQuery."  order by id DESC ");
...
{
?>
<tr align="center" <? if( $row["support_comment"]!="" && $row["final_status"]!=1 && $row["service_comment"]==""){ echo 'style="background-color:#D462FF"';} elseif($row["final_status"]==1){ echo 'style="background-color:#8BFF61"';}elseif($row["approve_status"]==1){ echo 'style="background-color:#B6B6B4"';}elseif( $row["admin_comment"]!="" && $row["final_status"]!=1 ){ echo 'style="background-color:#F2F5A9"';}?> >
<td><?php echo $i ?></td>
...
<td  style="width:200px"><a href="#" onclick="Show_record(<?php echo $row["id"];?>,'imei_change','popup1'); " class="topopup">View Detail</a>
| <a href="#" onclick="return ConfirmDelete(<?php echo $row["id"];?>);"  >Done</a>
   | <a href="#" onclick="return backComment(<?php echo $row["id"];?>);"  >Back Comment</a></td>
