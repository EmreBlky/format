...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=vehicle_no_changeclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=vehicle_no_changesupportComment",
...
if($_POST["Showrequest"]==1)
 {
	  $WhereQuery=" where user_id IN($user) and (approve_status=1 or approve_status!=1) and final_status=1";
 }
 ...
 {
	 $WhereQuery=" where (user_id IN($user) and support_comment!='' and final_status=0 and (service_comment is null or vehicle_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";
 } 
 else
 { 
	  
	 /* $WhereQuery=" where user_id IN($user) and ((approve_status=1) or (forward_back_comment is null and forward_req_user='".$_SESSION["user_name"]."') or (reason IN('Temperory no to Permanent no','Personal no to Commercial no','Commercial no to Personal no','For Warranty Renuwal Purpose'))) and final_status!=1 and (support_comment is null or service_comment!='')";*/
	 
	 $WhereQuery=" where (user_id IN($user) and ((approve_status=0 and reason IN('Temperory no to Permanent no','Personal no to Commercial no','Commercial no to Personal no','For Warranty Renuwal Purpose')) or approve_status=1) and final_status!=1 and (support_comment is null or service_comment!='') and vehicle_status=1 ) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment='')) ";
...
?>
        ...
        <td 
 style="width:200px"><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'vehicle_no_change','popup1'); " class="topopup">View Detail</a>
