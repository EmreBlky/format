...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Reactivation_of_accountclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Reactivation_of_accountsupportComment",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where (user_id IN($user) and  support_comment!='' and final_status=0 and (sales_comment is null or reactivation_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";

 } 
 else
 { 
	  
	 $WhereQuery=" where (user_id IN($user) and  (approve_status=1 and final_status!=1 and reactivation_status=1 and (support_comment is null or sales_comment!=''))) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
?>
	...
	$rowuser=select_query($sql);
        ...
        <td  style="width:200px"><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'reactivation_of_account','popup1'); " class="topopup">View Detail</a>
