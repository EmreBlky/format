...
if(isset($_GET['action']) && $_GET['action']=='sim_changeclose')
{
    ...
    $update_inventory = select_query_inventory("update inventory.sim set SimRemoveDate='".$check_query[0]['date']."', sim_status=92, active_status=1, is_testsim=0, status=0, flag=3, SimChangeRemarks='Support Team Send - ".$check_query[0]['reg_no']." - SIM changed' where phone_no='".$check_query[0]['old_sim']."'");
...
if(isset($_GET['action']) && $_GET['action']=='ActivateUserAccount')
{
        ...
        $row=select_query_live_con($query);
...
If($tablename=="new_account_creation")
   {
		...
		$ModelData = select_query("select * from new_account_model_master where is_active='1' and new_account_reqid='".$RowId."' ");
...
?>
    ...
    ?>
        ...
        <tr>
          ...
          <td><?php $vechile_no = explode(",",$row[0]["reg_no_of_vehicle_to_move"]);
for($i=0;$i<=count($vechile_no);$i++){ if($i%3!=0){ echo $vechile_no[$i].", ";}else { echo "<br/>".$vechile_no[$i].", ";} }?></td>
        </tr>
        <tr>
          <td>Contact Person </td>
...
<div id="databox">
  ...
  <div class="dataleft">
    ...
    ?>
        <tr>
          <td><strong>Replaced Device Info</strong></td>
          <td>---------------------------</td>
        </tr>
        <tr>
          <td>Client User</td>
...
<? }
    ...
    elseIf($tablename=="new_device_addition")
        ...
        $ffc_query = select_query_inventory("select ffc_reason from inventory.device_replace_ffc where imei_no='".$row[0]["device_imei"]."' order by id desc");
...
$biliing_status=$row[0]["billing_if_old_device"];
    ...
    ?>
        ...
        <?php if(count($ffc_query) > 0){?>
        <tr>
          <td><strong>FFC Reason </strong></td>
          <td><?echo $ffc_query[0]["ffc_reason"];?></td>
        </tr>
        <?php } ?>
      </tbody>
    </table>
