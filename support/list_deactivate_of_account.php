...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=deactivation_of_accountclose",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=deactivation_of_accountsupportComment",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where (user_id IN($user) and support_comment!='' and final_status=0 and (sales_comment is null or deactivation_status=2)) or (forward_req_user='".$_SESSION["user_name"]."' and forward_back_comment!='')";
 } 
 else
 { 
	  
	 $WhereQuery=" where (user_id IN($user) and (approve_status=1 and final_status!=1 and deactivation_status=1 and (support_comment is null or sales_comment!=''))) or (forward_req_user='".$_SESSION["user_name"]."' and (forward_back_comment is null or forward_back_comment=''))";
...
?>
 ...
 <? $sql="SELECT Userid AS id,UserName AS sys_username FROM addclient  WHERE Userid=".$query[$i]["user_id"];
  ...
  <td><?php echo $query[$i]["reason"];?></td>
   ...
   <td  style="width:200px"><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'deactivation_of_account','popup1'); " class="topopup">View Detail</a>
...
<td><?php if( $_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2 ){?> <a href="forward_sim_deactivation.php?id=<?php echo $query[$i]["id"];?>">Account Forward</a><?php }?> </td>
