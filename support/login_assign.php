...
$assignto = select_query("select * from login_user where parent_id='".$_SESSION['ParentId']."' and user_name!='".$_SESSION['user_name']."' "); 
...
if($action=='edit' or $action=='editp')
    {
        $result=select_query("select * from login_user_temp where id=$id");   
    }
?>
...
if(isset($_POST["submit"]))
{
    ...
    $tech_support_id = $_POST["tech_support_id"];
	$reply_id = $_SESSION['email'];
       

 if($action=='edit')
    {
        $query="update login_user_temp set user_name='".$user_name."',assign_to='".$assign_to."',password='".$password."',email='".$email_id."',from_date='".$from_date."',to_date='".$to_date."',reason='".$TxtReason."' where id=$id";
    ...
    {
        $query="INSERT INTO login_user_temp (login_user_id, user_name, password, assign_to, email, parent_id, branch_id, tech_support_id, from_date, to_date, reason)
    VALUES('".$login_user_id."','".$user_name."','".$password."','".$assign_to."','".$email_id."','".$parent_id."','".$branch_id."','".$tech_support_id."','".$from_date."','".$to_date."','".$TxtReason."')";
    ...
    if($insert_query)
    {
		...
		$textTosend.='<br>URL:- http://trackingexperts.com/format/index.php';
...
function validateForm()
{
  ...
  <form name="myForm" action="" onSubmit="return validateForm()" method="post">
    <table width="589" cellpadding="5" cellspacing="5" style=" padding-left: 100px;width: 550px;">
      <tr>
        <td colspan="2"><input type="hidden" name="login_user_id" value="<?=$_SESSION['userId'];?>" />
          <input type="hidden" name="parent_id" value="<?=$_SESSION['ParentId'];?>" />
          <input type="hidden" name="branch_id" value="<?=$_SESSION['BranchId'];?>" />
          <input type="hidden" name="tech_support_id" value="<?=$_SESSION['support_group_id'];?>" /></td>
      </tr>
      <tr>
        <td><label  id="lbDlUserName">User Name </label></td>
        <td><input type="text" name="user_name" id="user_name" readonly value="<?=$_SESSION['user_name'];?>"/></td>
      </tr>
      <tr>
        <td><label  id="lbDlAssignTo">Assign To </label></td>
        <td><select name="assign_to" id="assign_to">
        	<option value="">Select Person</option>
            <?php for($p=0;$p<count($assignto);$p++) {?>
            	<option value="<?=$assignto[$p]['user_name']?>" <? if($result[0]['assign_to']==$assignto[$p]['user_name']) {?> selected="selected" <? } ?>><?=$assignto[$p]['user_name'];?></option>
            <?php } ?>
        </select>
        </td>
      </tr>
      <tr>
        ...
        <td><input type="text" name="password" id="password"  value="<?=$result[0]['password'];?>"/></td>
      </tr>
      <tr>
        <td><label  id="lbDlEmailId">Email ID </label></td>
        <td><input type="text" name="email_id" id="email_id" value="<?=$result[0]['email'];?>"/></td>
      </tr>
      <tr>
        <td><label  id="lbDlFromDate">From date </label></td>
        <td><input type="text" name="from_date" id="datepicker" value="<?=$result[0]['from_date']?>" /></td>
      </tr>
      <tr>
        <td><label  id="lbDlToDate">To date </label></td>
        <td><input type="text" name="to_date" id="datepicker2" value="<?=$result[0]['to_date']?>" /></td>
      </tr>
      <tr>
        <td><label  id="lblReason">Reason</label></td>
        <td><textarea rows="5" cols="25"  type="text" name="reason" id="reason" > <?=$result[0]['reason']?>
</textarea></td>
      </tr>
      <tr>
