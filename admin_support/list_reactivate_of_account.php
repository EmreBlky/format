...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Reactivation_of_accountapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Reactivation_of_accountadminComment",
...
<div class="table">
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and (sales_comment is null or reactivation_status=2) and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 ...
 { 
	 $WhereQuery=" where approve_status=0 and (account_comment!='' or pay_pending!='') and (admin_comment is null or sales_comment!='') and reactivation_status=1 and (forward_req_user is null or forward_back_comment!='') and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales') and (service_comment!='' or service_comment is null) ";
   
 }
 

$query = select_query("SELECT * FROM reactivation_of_account ". $WhereQuery."   order by id desc ");   
...
?>
	...
	$rowuser=select_query($sql);
        ...
        <td   ><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'reactivation_of_account','popup1'); " class="topopup">View Detail</a>
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=19&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php  }?>
  </table>
