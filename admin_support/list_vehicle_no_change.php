...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=vehicle_no_changeapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=vehicle_no_changeadminComment",
...
if($_POST["Showrequest"]==1)
 {
	  $WhereQuery=" where (approve_status=1 or approve_status!=1) and final_status=1 and acc_manager!='triloki'";
 }
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and acc_manager!='triloki' and (service_comment is null or vehicle_status=2)";
 }
 ...
 { 
	 $WhereQuery=" where  approve_status=0 and acc_manager!='triloki' and (admin_comment is null or service_comment!='') and vehicle_status=1 and (account_comment!='' or payment_status!='') and (forward_req_user is null or forward_back_comment!='')";
...
?>
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=3&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
