...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=del_form_debtorsapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=del_form_debtorsadminComment",
...
<div class="top-bar">
  ...
  <br/>
  <div style="float:right";><font style="color:#F2F5A9;font-weight:bold;">Yellow:</font>Action Taken</div>
  <br/>
...
if($_POST["Showrequest"]==1)
 ...
 {
 $WhereQuery=" where admin_comment!='' and approve_status=0 and (sales_comment is null or del_debtors_status=2)";
  }
...
$WhereQuery=" where approve_status=0 and (account_comment!='' or total_pending!='') and del_debtors_status=1 and (admin_comment is null or sales_comment!='' ) and (forward_req_user is null or forward_back_comment!='')";	   
...
?>
 ...
 -->
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=13&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
