...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=sub_user_creationapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=sub_user_creationadminComment",
...
if($_POST["Showrequest"]==1)
 {
	  $WhereQuery=" where approve_status=1 and final_status=1 and acc_manager IN 	
	  ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 else if($_POST["Showrequest"]==2)
 {
	 $WhereQuery="where acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 else if($_POST["Showrequest"]==3)
 {
	 $WhereQuery=" where approve_status=1 and final_status!=1 and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 else if($_POST["Showrequest"]==4)
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and (sales_comment is null or sub_user_status=2) and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 else if($_POST["Showrequest"]==6)
 {
	 $WhereQuery=" where approve_status=0  and forward_req_user!='' and forward_comment!='' and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales')";
 }
 else
 { 
	 $WhereQuery=" where approve_status=0 and (admin_comment is null or sales_comment!='') and sub_user_status=1 and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb','jaipursales') and (forward_req_user is null or forward_back_comment!='')";
...
?>
  ...
  <td><?php echo $query[$i]["reason"];?></td> -->
        ...
        <td ><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'sub_user_creation','popup1'); " class="topopup">View Detail</a>
          ...
          Approved
          <?}else{?>
          <a href="#" onclick="return ConfirmDelete(<?php echo $query[$i]["id"];?>);"  >Approve</a> | <a href="#" onclick="return backComment(<?php echo $query[$i]["id"];?>);"  >Back Comment</a>
          <?}?>
          <?php } ?></td>
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=11&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
