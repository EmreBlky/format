...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=devicechangeadminComment",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery="where admin_comment!='' and approve_status=0 and acc_manager!='triloki' and (service_comment is null or device_change_status=2)";
 }
 else if($_POST["Showrequest"]==6)
 {
	 $WhereQuery=" where approve_status=0 and acc_manager!='triloki' and forward_req_user!='' and forward_comment!=''";
 }
 else
 { 
	  
	 $WhereQuery=" where approve_status=0 and acc_manager!='triloki' and (admin_comment is null or (service_comment!='' or 		
	 service_support_com!='')) and device_change_status=1 and (account_comment!='' or pay_status!='') 
	 and ((rdd_device_type!='New') or (rdd_device_type='New' and service_support_com!='')) 
	 and (forward_req_user is null or forward_back_comment!='')";
...
?>
      <tr align="center" <? if( $query[$i]["support_comment"]!="" && $query[$i]["final_status"]!=1 ){ echo 'style="background-color:#68C5CA"';} elseif($query[$i]["final_status"]==1){ echo 'style="background-color:#99FF66"';}elseif($query[$i]["approve_status"]==1){ echo 'style="background-color:#B6B6B4"';}elseif( $query[$i]["admin_comment"]!="" && $query[$i]["final_status"]!=1 && $query[$i]["service_comment"]==""){ echo 'style="background-color:#F2F5A9"';}?> >
        ...
        <td><?php echo $query[$i]["service_support_com"];?></td>
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=1&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php  }?>
  </table>
