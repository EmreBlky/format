...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=stop_gpsapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=stop_gpsadminComment",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and (sales_comment is null or stop_gps_status=2) and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb')";
 }
 ...
 { 
	 $WhereQuery=" where approve_status=0 and (account_comment!='' or total_pending!='') and stop_gps_status=1 and acc_manager IN ('pankaj','jaipurrequest','asaleslogin','ksaleslogin','msaleslogin','jsaleslogin','sanjeeb') and (admin_comment is null or sales_comment!='') and (forward_req_user is null or forward_back_comment!='')";
...
?>
  ...
  <td><?php echo $query[$i]["sales_action"];?></td> -->
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=10&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
