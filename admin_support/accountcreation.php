...
<div class="table">
 ...
 {
	  $WhereQuery=" where approve_status=1 and final_status=1 and branch_id!=1";
 }
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and branch_id!=1 and (sales_comment is null or acc_creation_status=2)";
 }
 else if($_POST["Showrequest"]==6)
 {
	 $WhereQuery=" where approve_status=0 and branch_id!=1 and forward_req_user!='' and forward_comment!=''";
 }
 else
 { 
	  
	 $WhereQuery=" where approve_status=0 and branch_id!=1 and (admin_comment is null or sales_comment!='') and acc_creation_status=1 and (forward_req_user is null or forward_back_comment!='')";
...
?>
      <tr align="center" <? if( $query[$i]["support_comment"]!="" && $query[$i]["final_status"]!=1 ){ echo 'style="background-color:#68C5CA"';} elseif($query[$i]["final_status"]==1){ echo 'style="background-color:#99FF66"';}elseif($query[$i]["approve_status"]==1){ echo 'style="background-color:#B6B6B4"';}elseif( $query[$i]["admin_comment"]!="" && $query[$i]["final_status"]!=1 && $query[$i]["sales_comment"]==""){ echo 'style="background-color:#F2F5A9"';}?> >
        ...
        <?php
		  ...
		  else if($query[$i]["mode_of_payment"]=="Cheque" || $query[$i]["mode_of_payment"]=="Billed") {
        ...
        <td><?php echo $query[$i]["rent_month"];?></td>
        <td><?php echo $query[$i]["rent_status"];?></td>
        <td><?php echo $query[$i]["dimts_fee"];?></td>
