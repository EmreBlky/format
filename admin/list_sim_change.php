...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=sim_changeapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=sim_changeadminComment",
...
<div class="table">
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and service_comment is null";
 }*/
 else if($_POST["Showrequest"]==6)
 {
	 //$WhereQuery=" where approve_status=0  and forward_req_user!='' and forward_comment!=''";
	 $WhereQuery=" where forward_req_user!='' and forward_comment!=''";
 }
 else
 {
 ...
 /*else
 { 
	 $WhereQuery=" where (approve_status=0 or approve_status=2) and (account_comment!='') and (admin_comment is null or service_comment!='') and (forward_req_user is null or forward_back_comment!='')";
...
?>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'sim_change','popup1'); " class="topopup">View Detail</a>
          <?php if($_POST["Showrequest"]!=2){?>
          
          <!--| <? if($query[$i]["approve_status"]==1){?>Approved<?}else{?><a href="#" onclick="return ConfirmDelete(<?php echo $query[$i]["id"];?>);"  >Approve</a><?}?> --> 
          <!--| <a href="#" onclick="return backComment(<?php echo $query[$i]["id"];?>);"  >Back Comment</a>-->
          
          <?php }?></td>
        <td><?php if($_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=4&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
