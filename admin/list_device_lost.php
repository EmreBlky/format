...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=device_lostapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=device_lostadminComment",
...
if($_POST["Showrequest"]==1)
 ...
 {
	 $WhereQuery=" where admin_comment!='' and approve_status=0 and (service_comment is null or device_lost_status=2)";
 }
 ...
 { 
	 ...
	 $WhereQuery=" where approve_status=0  and (account_comment!='' or odd_paid_unpaid!='') and (admin_comment is null or service_comment!='') and device_lost_status=1 and (forward_req_user is null or forward_back_comment!='')";
...
?>
        ...
        <td><a href="#" onclick="Show_record(<?php echo $query[$i]["id"];?>,'device_lost','popup1'); " class="topopup">View Detail</a>
        ...
        <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
          <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=5&height=220&width=330" class="thickbox"  >Froward Request</a>
          <?php } ?></td>
      </tr>
      <?php }?>
  </table>
