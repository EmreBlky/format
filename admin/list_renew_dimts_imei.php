...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Renew_dimts_imeiapprove",
 		data:"row_id="+row_id,
...
$.ajax({
		type:"GET",
		url:"userInfo.php?action=Renewdimts_imeiadminComment",
...
<div class="top-bar">
 ...
 {
	 $WhereQuery=" where approve_status=1 and final_status=0 and service_comment is null";
 }
  else if($_POST["Showrequest"]==4)
 {
	 $WhereQuery=" where admin_comment!='' and renew_dimts_status=2";
 }
 ...
 { 
	 $WhereQuery=" where approve_status=0 and (payment_status!='') and (admin_comment is null or service_comment!='') and renew_dimts_status=1 and (forward_req_user is null or forward_back_comment!='')";
...
?>
          ...
          <td><?php if($_POST["Showrequest"]!=1 && $_POST["Showrequest"]!=2){?>
            <a href="forwardrequest-iframe.php?id=<?= $query[$i]["id"]?>&req_id=20&height=220&width=330" class="thickbox"  >Froward Request</a>
            <?php } ?></td>
        </tr>
        <?php }?>
    </table>
