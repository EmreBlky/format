...
if($new_name!="" || $new_veh_reg!="" )
  ...
  $query1=("UPDATE services SET name='".$row['name']."', veh_reg='$veh_reg', Notwoking='$Notwoking', location='$location', atime='$atime',atimeto='$atimeto',atime_status='$atime_status', pname='$pname',  cnumber='$cnumber', pending='0' ,required='".$required."',datapullingtime='".$datapullingtime."',IP_Box='".$IP_Box."' ".$setqry.",device_model='$device_model' WHERE id='$id'");
		}
mysql_query($query1);
...
function req_info()
{
   ...
   if(document.form1.cnumber.value =="")
  {
   ...
   return false;
     ...
     if(document.form1.device_model.value==0)
  {
...
<td height="32" align="right">Notwoking:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td colspan="2"><input type="text" name="Notwoking" id="Notwoking" value="<?php echo $rows['Notwoking'];?>" readonly/></td>
</tr>
<tr>
...
<option value="">Select Status</option> 
			<option value="Till" <? if($rows['atime_status']=='Till') {?> selected="selected" <? } ?>> Till</option>
			<option value="Between" <? if($rows['atime_status']=='Between') {?> selected="selected" <? } ?>> Between</option>
...
<td colspan="2">
	 <input type="text" name="atime" id="atime" value="<?php echo $rows['atime'];?>" style="width:147px" autocomplete="off" readonly/> TO:- <input type="text" name="atimeto" id="atimeto" value="<?php echo $rows['atimeto'];?>" style="width:147px" autocomplete="off" readonly/> 		
	  	</td>
</tr>
<tr>
...
<? while($row=mysql_fetch_array($result)) { ?>
<option value=<?=$row['device_model']?><? if($device_model==$row['device_model']) { ?> selected="selected" <? }?>><?=$row['device_model']?></option>
<? } ?>
</select></td>
</tr>
