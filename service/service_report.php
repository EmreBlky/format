...
$reason_entry= $_POST['reason_entry'];
$FromDate= $_POST['Fromdate'];
$ToDate= $_POST['Todate'];
$job= $_POST['job'];
 
$Result_service = mysql_query("SELECT * FROM `services` where `reason`= '".$reason."' and time>= '".$FromDate."' and time <= '".$ToDate."'");
...
<?php 
$query="SELECT id,reason FROM reason where reason_for='service'";
$result=mysql_query($query);
?> 
<? $name1=$rows['reason']; ?> 
<td><select name="reason" style="width:147px" ><option value="0">Select Reason</option>
<? while($row=mysql_fetch_array($result)) { ?>
<option value="<?=$row['reason']?>" <? if($name1==$row['reason']) { ?> selected <? }?>><?=$row['reason']?></option>
<? } ?>
</select>  or 
<input type="text" name="reason_entry" id="reason_entry" value="<? if(isset($_POST["reason_entry"])){echo $_POST["reason_entry"];}?>" style="width:147px"  /> 
...
<tr>
	...
	{  echo  $_POST['reason'] . " or ". $reason_entry." -- " .mysql_num_rows($Result_service);
	}?> </font></td>
</tr>
</table>
<br/><br/>
...
<th width="11%" align="center"><font color="#0E2C3C"><b>Installer</b></font></th> 
<th width="11%" align="center"><font color="#0E2C3C"><b>Reason</b></font></th>
<th width="11%" align="center"><font color="#0E2C3C"><b>Status</b></font></th>
  ...
  if(isset($Result_service))
  {
	...
	<tr align="Center">
		...
		<td width="12%" align="center">&nbsp;<?php echo $row['atime'];?></td>
		<td width="12%" align="center">&nbsp;<?php echo $row['location'];?></td>

		 
		<td width="10%" align="center">&nbsp;<?php echo $row['inst_name'];?></td>		 
		<td width="12%" align="center">&nbsp;<?php echo $row['reason'];?></td>


		<!-- ---close -->

	 
		<td width="12%" align="center">&nbsp;Closed</td>
