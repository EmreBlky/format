...
if($mode=='') { $mode="new"; }
  ...
  if($mode=='close')
	{
	$rs = mysql_query("SELECT * FROM services where reason!='' and time!='' and branch_id=".$_SESSION['branch_id'] ." order by id desc");
	}
	else if($mode=='new')
	{
	//$rs = mysql_query("SELECT * FROM services  where inst_name!='' and inst_cur_location!='' and newpending!='1' order by id desc");
	$rs = mysql_query("SELECT * FROM services  where reason='' and time='' and inst_name!='' and inst_cur_location!='' and newpending!='1' and branch_id=".$_SESSION['branch_id'] ." order by id desc");
	}	
...
<table width="850"   cellpadding="2" cellspacing="3"><tr><td height="15px"><a href="newinstallation.php?mode=new">New</a></td><td height="15px"><a href="newinstallation.php?mode=close">Closed</a></td> </tr>
<tr></table>
...
<tr align="Center">
	...
	<tr align="Center" <? if($row['reason'] && $row['time'])  {  ?> style="background:#CCCCCC;" <? }?>>
		...
		<td width="10%" align="center">&nbsp;<?php echo @date('d-M-Y h:m:s',@strtotime($row['Notwoking']));?></td>
		<td width="7%" align="center">&nbsp;<?php echo $row['location'];?></td>
		<td width="10%" align="center">&nbsp;<?php echo $row['atime_status'];?></td>
		<td width="8%" align="center">&nbsp;<?php echo "$row[atime] -- $row[atimeto]";?></td>
